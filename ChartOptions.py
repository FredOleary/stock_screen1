...
class ChartOptions:
    ...
    def create_bid_ask_bar_chart(self,
        ...
        fig.suptitle(f"OTM Bid/Ask for {symbol}, ({symbol_name}), expires {expiration_date.strftime('%Y-%m-%d')}"
    ...
    def create_delta_theta_bar_chart(self,
                                 fig: Figure,
                                 symbol: str,
                                 symbol_name: str,
                                 expiration_date: datetime.datetime,
        ...
        rects1 = ax.bar(x - width / 2, delta_prices, width, label='Delta')
        rects2 = ax.bar(x + width / 2, theta_prices, width, label='Theta')
        ...
        fig.suptitle(f"OTM Delta/Theta for {symbol}, ({symbol_name}), expires {expiration_date.strftime('%Y-%m-%d')}"
                     f" ({days_to_expiration}), Current Price: {current_price}")
