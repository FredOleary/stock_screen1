...
class WebTradier:
                    ...
                    if is_third_friday and look_a_heads > 0:
                        ...
                        if bool(option_chain):
                            ...
                            if put_call == "BOTH" or put_call == "CALL":
                                df = self._filter_options( option_chain, "CALL", strike_filter, opt_chain['current_value'])
                                opt_chain['options_chain']["calls"] = df
                            if put_call == "BOTH" or put_call == "PUT":
                                df = self._filter_options( option_chain, "PUT", strike_filter, opt_chain['current_value'])
    ...
    def get_options_for_symbol_and_expiration(self, symbol: str, expiration: str,
                                              strike_filter="ATM", put_call="BOTH") -> {}:
        opt_chain = {}
        if self.logger is not None:
    ...
    def _filter_options( self, option_chain, put_call, strike_filter, current_value ) -> pd.DataFrame:
